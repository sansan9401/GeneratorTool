...
if which condor_submit 2>/dev/null|grep -q condor
then
    echo "[GeneratorTool] using condor"
...
if [ ! -d "$GENERATORTOOL_BASE/external/CMSSW_10_6_0" ]
then
    echo "[GeneratorTool]   No CMSSW_10_6_0; Cloning CMSSW_10_6_0;"
    cd $GENERATORTOOL_BASE/external
    cmsrel CMSSW_10_6_0
    cd CMSSW_10_6_0/src/
...
if [ ! -d "$GENERATORTOOL_BASE/external/genproductions" ]
then
    echo "[GeneratorTool]   No gen productions; Cloning genproductions;"
    cd $GENERATORTOOL_BASE/external
    git clone https://github.com/cms-sw/genproductions
    EXECUTABLE=$GENERATORTOOL_BASE/external/genproductions/bin/MadGraph5_aMCatNLO/gridpack_generation.sh
...
then
    ...
    cd $GENERATORTOOL_BASE/external/CMSSW_10_6_0/src
    eval `scramv1 runtime -sh`
    cd $GENERATORTOOL_BASE
fi
